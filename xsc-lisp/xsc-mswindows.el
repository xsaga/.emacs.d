...
(let ((git-usr-bin-path "C:/Program Files/Git/usr/bin"))
...
(if (file-readable-p (concat (file-name-as-directory everything-path) "Everything.exe"))
...
(if (file-exists-p "c:/msys64/msys2_shell.cmd")
  ...
  (progn
    ...
    (with-temp-buffer
      (if (or (file-exists-p (expand-file-name "~/aspell_autogenerated.cmd")) ;; or shortcut evitar llamar wsl cada vez que inicia emacs
	      (= 0 (call-process "wsl.exe" nil t nil "aspell" "--version")))
	  (progn
